...
app.get("/lists/:todoListId", (req, res, next) => {
  ...
  let list = loadTodoList(+todoListId, req.session.todoLists);
  if (list === undefined) {
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!list) {
        ...
        list.title = req.body.todoListTitle;
        req.flash("success", "Title succesfully changed!");
        res.redirect("/lists");
      }
    }
  }
...
app.post("/lists/:todoListId/todos/:todoId/toggle", (req, res, next) => {
  ...
  if (!todo) {
    ...
    if (todo.isDone()) {
      req.flash("success", `${todo.title} has now been marked NOT done`);
      todo.markUndone();
    } else {
      req.flash("success", `${todo.title} has now been marked done`);
      todo.markDone();
    }
  }
...
app.post("/lists/:todoListId/complete_all", (req, res, next) => {
  ...
  if (!list) {
    ...
    list.markAllDone();
    req.flash("success", `All tasks completed.`);
    res.redirect(`/lists/${todoListId}`);
  }
});
...
app.post("/lists/:todoListId/destroy", (req, res, next) => {
  ...
  if (!list) {
    ...
    req.flash("success", `${list.title} has been deleted.`);
    res.redirect("/lists");
  }
});
